...
def prepare_data(df):
    ...
    return df.drop(columns=["ratings"]), top_values

df, top_values = prepare_data(df)

@st.cache_resource(hash_funcs={pd.DataFrame: id, list: id})
def linear_regression(df, columns):
    ...
    return df, model_rating, model_rating_var, (X_train, X_test, y_train, y_test)
...
df, model_rating, model_rating_var, train_test = linear_regression(df, columns)
...
st.write(f"Błąd średniokwadratowy dla średniej oceny: {mse_rating:.4f}")
st.write(f"Błąd średniokwadratowy dla wariancji ocen: {mse_rating_var:.4f}")

def sum_coefficients(coef, top_values, numeric_cols, columns):
    coef_sum = {}
    for column in top_values.keys():
        if column in columns:
...
coef_sum_rating = sum_coefficients(coef_rating, top_values, numeric_cols, columns).sort_values(ascending=False)
coef_sum_rating_var = sum_coefficients(coef_rating_var, top_values, numeric_cols, columns).sort_values(ascending=False)
